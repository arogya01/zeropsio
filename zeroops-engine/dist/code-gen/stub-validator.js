...
exports.validateTsAst = validateTsAst;
exports.validateGoSyntax = validateGoSyntax;
exports.validateNonTsFile = validateNonTsFile;
exports.validateZeroStubs = validateZeroStubs;
const typescript_1 = __importDefault(require("typescript"));
/**
 * Validates JS/TS/TSX/JSX files using TypeScript Compiler API AST inspection.
 */
function validateTsAst(filePath, content) {
    const violations = [];
    let astValid = true;
    const isJsx = filePath.endsWith('.tsx') || filePath.endsWith('.jsx');
    const scriptKind = isJsx ? typescript_1.default.ScriptKind.TSX : typescript_1.default.ScriptKind.TS;
    let sourceFile;
    try {
        sourceFile = typescript_1.default.createSourceFile(filePath, content, typescript_1.default.ScriptTarget.Latest, true, scriptKind);
    }
    catch {
        return {
            astValid: false,
    ...
    const scanner = typescript_1.default.createScanner(typescript_1.default.ScriptTarget.Latest, false, typescript_1.default.LanguageVariant.Standard, content);
    let token = scanner.scan();
    while (token !== typescript_1.default.SyntaxKind.EndOfFileToken) {
        if (token === typescript_1.default.SyntaxKind.SingleLineCommentTrivia || token === typescript_1.default.SyntaxKind.MultiLineCommentTrivia) {
 ...
 */
function validateNonTsFile(filePath, content) {
    const violations = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isPolyglotStub = /\b(TODO|STUB|FIXME|XXX|HACK|NOT[_\s]IMPLEMENTED|UNIMPLEMENTED)\b/i.test(line) ||
 ...
 */
function validateZeroStubs(files) {
    const allViolations = [];
    const stubsFound = [];
    ...
    for (const [filePath, content] of Object.entries(files)) {
        ...
        if (['ts', 'tsx', 'js', 'jsx'].includes(ext)) {
            ...
            if (nonTsViolations.some((v) => v.rule.includes('UNTERMINATED') ||
...
//# sourceMappingURL=stub-validator.js.map