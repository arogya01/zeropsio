...
class LiveAuditor {
     ...
     */
    async runFullAudit(url, projectName, onLogStream) {
        ...
        if (onLogStream)
            onLogStream(`[TEST-1] HTTP GET ${url} ...`);
        ...
        if (onLogStream)
            onLogStream(`[TEST-2] API Gateway Health Check: GET ${apiHealthUrl} ...`);
        ...
        if (publicHttpRes.ok) {
            ...
            errors.push(`HTTP status check failed with status ${publicHttpRes.status}`);
        }
        if (apiGatewayRes.ok) {
            passedCount++;
        }
        else {
            errors.push(`API Gateway health check failed with status ${apiGatewayRes.status}`);
        }
        if (dbRes.connected && dbRes.writeOk) {
            passedCount++;
        }
...
exports.LiveAuditor = LiveAuditor;
//# sourceMappingURL=live-auditor.js.map